 ...
 *
 * Every instance represents an account. If you want to connect to more than one
 * discord account you have to use more instances.
 */
public interface DiscordAPI {
	 ...
	 *
	 * @param callback
	 *            The callback will inform you when the connection is ready. The
	 *            connection is ready as soon as the ready packet was received.
	 */
	 ...
	 * Connects to the account with the given token or email and password.
	 *
	 * This method is blocking! It's recommended to use the non-blocking version
	 * which uses a thread from the internal used thread pool to connect.
	 */
	...
	/**
	 * Sets the game shown in the user list and a streaming url (which means it
	 * shows you as streaming).
	 *
	 * @param game
	 *            The game to set.
	 * @param streamingUrl
	 ...
	 * @return The server with the given id. <code>Null</code> if no server with
	 *         the id was found.
	 */
	 ...
	 * @return The channel with the given id. <code>Null</code> if no channel
	 ...
	 * @return The voice channel with the given id. <code>Null</code> if no
	 *         channel with the id was found.
	 */
	public VoiceChannel getVoiceChannelById(String id);

	/**
	 * Gets an user by its id. It first will check if the user is in the cache.
	 * If no user was found in the cache it tries to request it from the api.
	 *
	 * @param id
	 *            The id of the user.
	 * @return The user with the given id. <code>Null</code> if no user with the
	 *         id was found.
	 */
	Future<User> getUserById(String id, final boolean addToCache);

	/**
	 * Gets an user by its id. Unlike {@link #getUserById(String)} this method
	 * only search for the user in the cache. Some members of bigger servers may
	 * not be in the cache cause discord only sends the online users for servers
	...
	/**
	 * Gets a message by its id. This method may return <code>null</code> even
	 * if the message exists!
	 *
	 * @param id
	 *            The id of the message.
	 * @return The message with the given id or <code>null</code> no message was
	 *         found.
	 */
	 ...
	 * Don't use multi-threading if you don't know how to make things
	 * thread-safe or how to prevent stuff like deadlocks!
	 *
	...
	/**
	 * Gets the token of the current connection. It's recommended to store this
	 * token somewhere and use it to login instead of always connecting using
	 * email and password. Discord will block too many token requests.
	 *
	 * @return The token of the current connection. <code>Null</code> if the bot
	 *         isn't connected.
	 */
	public String getToken();

	/**
	 * Sets the token which is used to connect. You don't need email and
	 * password if you have a token, but it's recommended to set email and
	 * password, too. The api will try to connect using the token first. If this
	 * fails (e.g. if the token is expired) it will use the email and password.
	 *
	 * @param token
	 *            The token to set.
	 * @param bot
	 *            Whether the token is the token of a bot account or a normal
	 *            account.
	 */
	 ...
	 *            The invite code.
	 * @param callback
	 *            The callback which will be informed when you joined the server
	 *            or joining failed.
	 * @return The server.
	 */
	 ...
	 *            The name of the new server.
	 * @param callback
	 *            The callback which will be informed when you created the
	 *            server.
	 * @return The created server.
	 */
	...
	/**
	 * Updates the username. If you want to update the email or password, too,
	 * use {@link #updateProfile(String, String, String, BufferedImage)}.
	 * Otherwise the first update will be overridden (except you wait for it to
	 * finish using {@link Future#get()}).
	 *
	 * @param newUsername
	 *            The new username.
	...
	/**
	 * Updates the email address. Attention: Do not mix up with
	 * {@link #setEmail(String)}! This method changes the account settings! If
	 * you want to update the username or password, too, use
	 * {@link #updateProfile(String, String, String, BufferedImage)}. Otherwise
	 * the first update will be overridden (except you wait for it to finish
	 * using {@link Future#get()}).
	 *
	 * @param newEmail
	 *            The new email.
	 * @return A future which tells us whether the update was successful or not.
	...
	/**
	 * Updates the password. Attention: Do not mix up with
	 * {@link #setPassword(String)}! This method changes the account settings!
	...
	/**
	 * Updates the avatar. If you want to update the username, password or
	 * email, too, use
	 * {@link #updateProfile(String, String, String, BufferedImage)}. Otherwise
	 * the first update will be overridden (except you wait for it to finish
	 * using {@link Future#get()}).
	 ...
	 *
	 * @param newUsername
	 *            The new username. Set to <code>null</code> to keep the current
	 *            name.
	 * @param newEmail
	 *            The new email. Set to <code>null</code> to keep the current
	 *            email.
	 * @param newPassword
	 *            The new password. Set to <code>null</code> to keep the current
	 *            password.
	 * @param newAvatar
	 *            The new avatar. Set to <code>null</code> to keep the current
	 *            avatar.
	 * @return A future which tells us whether the update was successful or not.
	 */
	 ...
	 *
	 * @param invite
	 *            The invite code or the invite url.
	 ...
	 *            The invite code or the invite url.
	 * @param callback
	 *            The callback which will be informed when the invite has been
	 *            parsed.
	 * @return The parsed invite.
	 */
	...
	/**
	 * Sets the size of message cache. If the cache is full the oldest message
	...
	/**
	 * Gets the size of the message cache. If the cache is full the oldest
	 * message in the cache will be removed.
	 *
	...
	/**
	 * Gets a new permissions builder with every type set to
	 * {@link de.btobastian.javacord.entities.permissions.PermissionState#NONE}
	 *
	 ...
	 *
	 * @param autoReconnect
	 *            Whether the api should try to auto-reconnect or not.
	 */
	...
	/**
	 * Gets the rate limit manager. This class caches all rate limits of the
	 * api.
	 *
	 ...
	 *
	 * This value is <code>true</code> by default. If it's set to
	 * <code>false</code> the list of servers ({@link #getServers()}) will be
	 * empty after connecting and will be filled a few seconds later (depending
	 * on the amount of servers).
	 *
	 * @param wait
	 *            Whether the bot should wait for all servers to be loaded or
	 *            not.
	 */
	...
	/**
	 * Disconnects the bot. After disconnecting you should NOT use this instance
	 * again.
	 */
