...
public class ChannelUpdateHandler extends PacketHandler {
	...
	private void handleServerTextChannel(JSONObject packet, Server server) {
		...
		if ((channel.getTopic() != null && topic == null) || (channel.getTopic() == null && topic != null)
			...
			if (type.equals("member")) {
				...
				if (!oldPermissions.equals(permissions)) {
					...
					listenerExecutorService.submit(new Runnable() {
						@Override
						public void run() {
							List<UserChangeOverwrittenPermissionsListener> listeners = api
							...
							synchronized (listeners) {
								for (UserChangeOverwrittenPermissionsListener listener : listeners) {
									try {
										...
										logger.warn("Uncaught exception in UserChangeOverwrittenPermissionsListener!",
												t);
									}
								}
							}
			...
			if (type.equals("role")) {
				...
				if (!permissions.equals(oldPermissions)) {
					...
					listenerExecutorService.submit(new Runnable() {
						@Override
						public void run() {
							List<RoleChangeOverwrittenPermissionsListener> listeners = api
							...
							synchronized (listeners) {
								for (RoleChangeOverwrittenPermissionsListener listener : listeners) {
									try {
										...
										logger.warn("Uncaught exception in RoleChangeOverwrittenPermissionsListener!",
												t);
									}
								}
							}
	...
	private void handleServerVoiceChannel(JSONObject packet, Server server) {
		...
		if (!channel.getName().equals(name)) {
			...
			listenerExecutorService.submit(new Runnable() {
				@Override
				public void run() {
					List<VoiceChannelChangeNameListener> listeners = api
							.getListeners(VoiceChannelChangeNameListener.class);
					synchronized (listeners) {
		...
		if (channel.getPosition() != position) {
			...
			listenerExecutorService.submit(new Runnable() {
				@Override
				public void run() {
					List<VoiceChannelChangePositionListener> listeners = api
