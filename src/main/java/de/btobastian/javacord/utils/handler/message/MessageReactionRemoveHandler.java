	...
	public void handle(JSONObject packet) {
		...
		if (isCustomEmoji) {
			...
			if (message.isPrivateMessage()) {
				// Private messages with custom emoji? Maybe with Nitro, but
			...
			reaction = ((ImplMessage) message).removeCustomEmojiReactionToCache(customEmoji,
					api.getYourself().getId().equals(userId));
		} else {
			reaction = ((ImplMessage) message).removeUnicodeReactionToCache(emoji.getString("name"),
					api.getYourself().getId().equals(userId));
		}
